...
function MBF:OnInitialize()
	...
	-- Force our drag handlers via SetScript to ensure they fire
	MinimapButtonFrame:SetScript("OnDragStart", function()
		MBF:OnDragStart()
	end)
	MinimapButtonFrame:SetScript("OnDragStop", function()
		MBF:OnDragStop()
	end)
	-- Also set on the drag button
	MinimapButtonFrameDragButton:SetScript("OnMouseDown", function(self, button)
		if button == "LeftButton" then
			MBF:OnDragStart()
		end
	end)
	MinimapButtonFrameDragButton:SetScript("OnMouseUp", function(self, button)
		if button == "LeftButton" then
			MBF:OnDragStop()
		end
	end)

	-- Ensure mouse is enabled on these frames
	MinimapButtonFrame:EnableMouse(true)
	MinimapButtonFrameDragButton:EnableMouse(true)

	-- Also set frame strata high to avoid being blocked
	MinimapButtonFrameDragButton:SetFrameStrata("HIGH")

	-- Delayed check to fix anchor chains
	self:ScheduleTimer(function()
		-- FIX: Break any anchor chains at startup
		if MMHolder then
			local _, mmhRelTo = MMHolder:GetPoint()
			if mmhRelTo and mmhRelTo:GetName() == "MinimapButtonFrame" then
				local mmhX, mmhY = MMHolder:GetCenter()
				MMHolder:ClearAllPoints()
				MMHolder:SetPoint("CENTER", UIParent, "BOTTOMLEFT", mmhX, mmhY)
			end
		end

		-- Also check if MBF is anchored to anything minimap-related
		local mbfPoint, mbfRelTo = MinimapButtonFrame:GetPoint()
		if mbfRelTo then
			local relName = mbfRelTo:GetName()
			if relName == "MMHolder" or relName == "Minimap" or relName == "MinimapCluster" or relName == "MinimapMover" then
				local mbfX, mbfY = MinimapButtonFrame:GetCenter()
				MinimapButtonFrame:ClearAllPoints()
				MinimapButtonFrame:SetPoint("CENTER", UIParent, "BOTTOMLEFT", mbfX, mbfY)
			end
		end
	end, 10)

	-- Hook MMHolder:SetPoint to catch and block anything that tries to anchor it to MBF
	self:ScheduleTimer(function()
		if MMHolder and not MMHolder.__MBF_hooked then
			local originalSetPoint = MMHolder.SetPoint
			MMHolder.SetPoint = function(self, point, relativeTo, relPoint, x, y)
				local relName = relativeTo
				if type(relativeTo) == "table" and relativeTo.GetName then
					relName = relativeTo:GetName()
				end
				if relName == "MinimapButtonFrame" then
					-- Blocked: Something tried to anchor MMHolder to MBF - redirect to UIParent
					return originalSetPoint(self, point, UIParent, relPoint or point, x or 0, y or 0)
				end
				return originalSetPoint(self, point, relativeTo, relPoint, x, y)
			end
			MMHolder.__MBF_hooked = true
		end
	end, 11)
...
function MBF:OnDragStart()
	print("[MBF] OnDragStart called")
	print("[MBF] Parent:", MinimapButtonFrame:GetParent():GetName())
	local point, relativeTo, relativePoint = MinimapButtonFrame:GetPoint()
	print("[MBF] Anchor:", point, relativeTo and relativeTo:GetName() or "nil", relativePoint)

	-- FIX: Break any anchor chains in BOTH directions
	-- Case 1: MMHolder is anchored TO MBF
	if MMHolder then
		local _, mmhRelTo = MMHolder:GetPoint()
		if mmhRelTo and mmhRelTo:GetName() == "MinimapButtonFrame" then
			print("[MBF] Breaking MMHolder->MBF anchor chain!")
			local mmhX, mmhY = MMHolder:GetCenter()
			MMHolder:ClearAllPoints()
			MMHolder:SetPoint("CENTER", UIParent, "BOTTOMLEFT", mmhX, mmhY)
		end
	...
	-- Case 2: MBF is anchored TO MMHolder (or any minimap-related frame)
	if relativeTo then
		local relName = relativeTo:GetName()
		if relName == "MMHolder" or relName == "Minimap" or relName == "MinimapCluster" or relName == "MinimapMover" then
			print("[MBF] Breaking MBF->", relName, "anchor chain!")
			local mbfX, mbfY = MinimapButtonFrame:GetCenter()
			MinimapButtonFrame:ClearAllPoints()
			MinimapButtonFrame:SetPoint("CENTER", UIParent, "BOTTOMLEFT", mbfX, mbfY)
		end
	...
	if not MBF.db.profile.locked then
		-- Ensure we're parented to UIParent, not Minimap
		if MinimapButtonFrame:GetParent() ~= UIParent then
			print("[MBF] Reparenting to UIParent")
			MinimapButtonFrame:SetParent(UIParent)
		end
		print("[MBF] Calling StartMoving()")
		MinimapButtonFrame:StartMoving()
	else
		print("[MBF] Frame is locked, not moving")
	end
end
...
function MBF:OnDragStop()
	...
	local point, relativeTo, relativePoint, xOfs, yOfs = MinimapButtonFrame:GetPoint()
	-- Force anchor relative to UIParent if it somehow got anchored to Minimap
	if relativeTo and relativeTo:GetName() ~= "UIParent" then
		local centerX, centerY = MinimapButtonFrame:GetCenter()
		local uiWidth, uiHeight = UIParent:GetSize()
		MinimapButtonFrame:ClearAllPoints()
		MinimapButtonFrame:SetPoint("CENTER", UIParent, "BOTTOMLEFT", centerX, centerY)
		point, relativeTo, relativePoint, xOfs, yOfs = MinimapButtonFrame:GetPoint()
	end
	local relName = relativeTo and relativeTo:GetName() or "UIParent"
	MBF.db.profile.MBF_FrameLocation = { point, relName, relativePoint, xOfs, yOfs }
end
...
function MBF:ChatCommand(input)
	if not input or input:trim() == "" then
		...
		if (firstWord == "testdrag") then
			print("[MBF] === ANCHOR CHAIN ANALYSIS ===")
			-- Check MBF anchor
			local mbfPoint, mbfRelTo, mbfRelPoint = MinimapButtonFrame:GetPoint()
			print("[MBF] MBF anchored to:", mbfRelTo and mbfRelTo:GetName() or "nil")
			-- Check Minimap anchor
			local mmPoint, mmRelTo, mmRelPoint = Minimap:GetPoint()
			print("[MBF] Minimap anchored to:", mmRelTo and mmRelTo:GetName() or "nil")
			-- Check MMHolder if it exists
			if MMHolder then
				local mmhPoint, mmhRelTo, mmhRelPoint = MMHolder:GetPoint()
				print("[MBF] MMHolder anchored to:", mmhRelTo and mmhRelTo:GetName() or "nil")
			end
			-- Check if any frame is anchored TO MBF by checking common suspects
			print("[MBF] --- Checking if anything is anchored TO MinimapButtonFrame ---")
			local suspects = { Minimap, MinimapCluster, MinimapBackdrop }
			if MMHolder then table.insert(suspects, MMHolder) end
			for _, frame in ipairs(suspects) do
				if frame then
					local pt, rel = frame:GetPoint()
					if rel and rel:GetName() == "MinimapButtonFrame" then
						print("[MBF] WARNING:", frame:GetName(), "is anchored TO MinimapButtonFrame!")
					end
				end
			end
			print("[MBF] Testing OnDragStart manually...")
			MBF:OnDragStart()
			print("[MBF] Calling OnDragStop immediately...")
			MBF:OnDragStop()
			print("[MBF] Test complete.")
		end
		if (firstWord == "stopdrag") then
			MinimapButtonFrame:StopMovingOrSizing()
			print("[MBF] StopMovingOrSizing called")
		end
...
function MBFSkinMinimapButton(miniButton, override)
	...
	if not miniButton.MBFoTex then
		miniButton.MBFoTex = miniButton:CreateTexture("MBFOverlay");
	end
	...
	if (tBorder[MBF.db.profile.currentTexture] == nil) then
		...
		miniButton.MBFoTex:SetDrawLayer("OVERLAY");
	end

	if (override ~= 1) then
		if not miniButton.MBFbTex then
			miniButton.MBFbTex = miniButton:CreateTexture("MBFBackground");
		end
		...
		if (tBackdrop[MBF.db.profile.currentTexture] == nil) then
			...
			miniButton.MBFbTex:SetDrawLayer("BACKGROUND");
...
saveOldData = function(miniButton)
	...
	if oldPoint ~= nil then
		...
		MBF_ButtonInfoStorage[miniButton:GetName()] = { oldName, oldParentName, oldFrameLevel, oldPoint, oldRelativeTo,
			oldRelativePoint, oldXofs, oldYofs, oldOnDragStart, oldOnDragStop, oldOnEvent, oldWidth, oldHeight, oldScale,
			oldLevel, oldStrata };
		result = true;
	end
	return result;
	...
	if not isInTable(BlizzButtons, miniButton:GetName()) then
		for i, child in ipairs({ miniButton:GetChildren() }) do
			...
			if (childName ~= nil) then
				...
				MBF_ChildEventStorage[childName] = { childParentName, childName, childOnDragStart, childOnDragStop,
					childOnEvent };
...
restoreButtonSettings = function(miniButton)
	...
	if MBF_ButtonInfoStorage[buttonName] ~= nil then
		oldName, oldParentName, oldFrameLevel, oldPoint, oldRelativeTo, oldRelativePoint, oldXofs, oldYofs, oldOnDragStart, oldOnDragStop, oldOnEvent, oldWidth, oldHeight, oldScale, oldLevel, oldStrata =
			unpack(MBF_ButtonInfoStorage[buttonName]);
		...
		if ((buttonName ~= "MinimapButtonFrameDragButton") and (buttonName ~= "MiniMapMailFrameDisabled") and (miniButton ~= nil) and (((MinimapButtonFrame:IsShown()) and (buttonName == "MiniMapMailFrame") or (miniButton:IsShown())))) then
			...
			if (miniButton:GetScale() ~= 1) then
				if MBF.db.profile.GrowUp then
					miniButton:SetPoint("CENTER", MinimapButtonFrame, "BOTTOMLEFT", x + (x * (1 - miniButton:GetScale())),
						-y - (y * (1 - miniButton:GetScale())));
				else
					miniButton:SetPoint("CENTER", MinimapButtonFrame, "TOPLEFT", x + (x * (1 - miniButton:GetScale())),
						y + (y * (1 - miniButton:GetScale())));
				end
			else
				if MBF.db.profile.GrowUp then
...
function MBFC_LoadStandardSkins()
	...
	MBFAddSkin("Blizzard Square", "Interface\\AddOns\\MinimapButtonFrame\\Textures\\BlizzSquare.tga",
		"Interface\\Buttons\\UI-EmptySlot", 45);
end
