...
3.1.1	Fixed  - ElvUI Drag Conflict: MBF now drags independently without moving the minimap when ElvUI is enabled
	Fixed  - Anchor chain issues with MMHolder and ElvUI minimap frames
	New    - ElvUI Integration: New toggle in ElvUI settings (Maps → Minimap → Buttons → MinimapButtonFrame)
		 When enabled, ElvUI releases button control to MBF for seamless coexistence
	Change - Removed debug print statements from initialization and anchor fixing code
	Change - Added hook on MMHolder:SetPoint to prevent anchor loops with ElvUI
3.1.2	Fixed  - ElvUI minimap panels (LeftMiniPanel, RightMiniPanel, etc.) no longer hide when MBF frame is hidden
	Change - Added all 8 ElvUI MiniPanel frames to the BlizzUI protection list
3.1.3	New    - Added "Plugins" submenu in MBF options for addon integrations
	New    - ElvUI Integration toggle: "Let ElvUI Control Buttons" to relinquish control to ElvUI
	New    - Popup when toggling to ElvUI offers to disable MBF addon automatically
	Change - Both MBF and ElvUI now show reload popups when toggling control
...
3.1.5	Fixed  - Quest icon pins (from pfQuest and similar addons) no longer collected into MBF frame
	...
	Change - Improved detection of non-button minimap elements
